...
Seance::Seance(int id_exp,QString type,QString duree,QString date,QString heure)
...
bool Seance::ajouter()
{
    ...
    QString id_s= QString::number(id_seance);
     QString id_ex= QString::number(id_exp);
        ...
        query.prepare("INSERT INTO SEANCES (ID_EXPERT,DUREE,DATEE,TYPEE,heure) "
                      "VALUES ( :id_exp, :duree, :date, :type , :heure )");
...
bool Seance::modifier(int id)
{
    ...
    query.prepare("UPDATE SEANCES SET ID_SEANCE=:id,ID_EXPERT=:idexpert,DUREE=:duree,DATEE=:date,TYPEE=:type,heure=:heure WHERE ID_SEANCE=:id");
    query.bindValue(":id",ids);
